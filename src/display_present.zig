...
pub const Owner = struct {
    ...
    cleaned: bool = false,
    retired: bool = false,
    phase: enum { idle, copying, flip_retry, flip_wait, sample_retry, sample_wait, ack_retry, ack_wait, failed } = .idle,
    failure: ?anyerror = null,
    failed_output: bool = false,
    cancel_display: bool = false,
    ...
    pub fn bind(self: *Owner, engine: *sdma.Owner, core: *dc.Owner, frames: [2]*buffers.Image, epoch: dc.scanout.Epoch, target: a.GfxOutputTarget) !void {
        if (self.self_address != 0 or engine.self_address != @intFromPtr(engine) or !engine.registered or !engine.verified or
            ...
            core.scanoutFor(epoch).phase != .active or !std.meta.eql(core.scanoutFor(epoch).epoch, epoch) or !std.meta.eql(engine.binding, epoch.backend) or
            ...
            target.connection_generation != epoch.output.connection_generation or target.display_generation != epoch.display or target.head_id != core.scanoutFor(epoch).mode.pipe or
            ...
            core.scanoutFor(epoch).current.?.address != frames[0].mc_address or !std.meta.eql(core.scanoutFor(epoch).current.?.reference, frames[0].reference.reference)) return error.State;
        ...
        self.sequence = core.scanoutFor(epoch).sequence;
    }
    pub fn available(self: *const Owner) bool {
        return self.self_address == @intFromPtr(self) and self.phase == .idle and self.input == null and self.failure == null and
            self.core.?.thread == 0 and self.core.?.phase == .programmed and self.core.?.scanoutFor(self.epoch).phase == .active;
    }
    ...
    fn submit(self: *Owner) !void {
        ...
        const address = source_va + @as(u64, self.target.head_id) * buffers.max_bytes;
        const source_span: @import("memory_layout.zig").Span = .{ .offset = address, .bytes = buffers.max_bytes };
        if (source_span.overlaps(memory.layout.?.mc) or source_span.overlaps(memory.layout.?.gart)) return error.Invalid;
        try self.source.adopt(memory, &self.reference, address, self.pages[0..@intCast((descriptor.byte_length + 4095) / 4096)]);
    ...
    fn retire(raw: usize, fence: a.GfxFence) bool {
        ...
        if (self.cancel_display) {
            if (self.core.?.thread != 0 and self.core.?.taskFor(self.target.head_id, self.target.connector_id) and !self.core.?.poll()) return false;
            entry.result = a.gfx_queue_result_cancelled; self.retired = true; return true;
        }
    ...
    fn confirm(self: *Owner) !bool {
        const core = self.core.?; const life = core.scanoutFor(self.epoch);
        const waiting = self.phase == .flip_wait or self.phase == .sample_wait or self.phase == .ack_wait;
        // Every task result belongs to its exact submitting head. Never join
        // or consume a peer's mode/health/cursor/scanout operation.
        if (waiting) {
            if (core.action != .scanout_work or !std.meta.eql(core.scanout_request.epoch, self.epoch)) return error.Stale;
            if (!core.poll()) return false;
        } else if (core.thread != 0) return false;
        ...
        switch (self.phase) {
            ...
            .sample_wait => {
                ...
                const receipt = life.receipt orelse return error.Unconfirmed;
