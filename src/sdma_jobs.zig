...
const Job = struct {
    owner: ?*Owner = null, fence: a.GfxFence = .{}, ticket: ?q.Ticket = null,
    maps: [2]Mapping = .{ .{}, .{} }, references: [2]a.GfxBufferReference = .{ .{}, .{} },
    pages: [2][max_pages]u64 = undefined, retained: [2]bool = .{ false, false },
    submitted: bool = false, fence_retired: bool = false,
    fn retire(raw: usize, fence: a.GfxFence) bool {
        ...
        for (&self.maps, &self.retained) |*map, *held| {
            if (held.*) { map.complete(fence) catch return false; held.* = false; }
    ...
    fn clear(self: *Job) void {
        ...
        self.owner = null; self.fence = .{}; self.ticket = null;
        self.submitted = false; self.fence_retired = false;
    }
};
pub const Owner = struct {
    ...
    arena_pages: [storage.bytes / 4096]u64 = undefined, arena_translated: bool = false, arena_flush_pending: bool = false,
    gc_stop: @import("start_engines.zig").Park = .{}, gc_stop_started: bool = false, closed: bool = false,
    selftest_submitted: bool = false, verified: bool = false, deadline: c.Deadline = .{},
    pub fn prepare(self: *Owner, memory: *mem.Owner, runtime: *@import("queue_runtime.zig").Owner,
        native: *const @import("start_runtime.zig").Owner) Error!void
    {
        ...
        self.self_address = @intFromPtr(self); self.memory = memory; self.runtime = runtime;
        ...
        self.arena_translated = true; self.arena_flush_pending = true;
        try @import("memory_hubs.zig").flush(&memory.registers, 1); self.arena_flush_pending = false;
    ...
    fn selftest(self: *Owner) Error!void {
        ...
        const data = try runtime.arena.words32(test_offset, 32); for (data) |*word| word.* = 0;
        ...
        self.commands[count] = 0; count += 1; // drain outstanding SDMA memory operations
        const ib = try runtime.arena.ib(0);
        for (self.commands[0..count], 0..) |word, i| ib[i] = word;
        var commands: [32]u32 = undefined;
        _ = try @import("sdma_ring.zig").frame(&commands, self.engine.ring.write, arena_va + storage.ib_offset, count,
            try runtime.arena.address(test_offset, 8), test_token, false);
    ...
    pub fn pollSelftest(self: *Owner) Error!bool {
        ...
        const io = &self.memory.?.registers; _ = try self.deadline.check(io.nowNs());
        try c.hdpInvalidate(io); try self.engine.observe(io);
        const data = try self.runtime.?.arena.words32(test_offset, 32);
        const first = (@as(u64, data[1]) << 32) | data[0];
        try io.barrier(); const second = (@as(u64, data[1]) << 32) | data[0];
    ...
    pub fn activate(self: *Owner, native: *const @import("start_runtime.zig").Owner) Error!void {
        ...
        const ctx = native.ctx.?; const queue = ctx.graphicsQueue() orelse return error.Unsupported; self.queue = queue;
        const amd = @import("r4amd");
        const details: amd.R4AmdDriverProfile = .{ .version = 1, .size = @sizeOf(amd.R4AmdDriverProfile), .vendor_id = amd.vendor_id,
            .device_id = 0x15d8, .gc_version = amd.gc_9_1_0, .sdma_version = amd.sdma_4_1_0, .command_abi = amd.command_abi, .reserved = 0 };
        var profile: a.GfxBackendProfile = .{ .interface_id_lo = amd.backend_v1_header.interface_id_lo,
        ...
        if (queue.registerProfile(&.{ .adapter_id = self.memory.?.adapter, .memory_generation = self.memory.?.epoch, .milestone = a.gfx_queue_milestone_device_execution,
    ...
    pub fn close(self: *Owner) bool {
        ...
        if (self.arena_translated) {
            ...
            self.arena_translated = false; self.arena_flush_pending = true;
        }
        if (self.arena_flush_pending) {
            @import("memory_hubs.zig").flush(&memory.registers, 1) catch return false; self.arena_flush_pending = false;
        }
        if (self.registered) {
        ...
        self.active = false; self.closed = true; return true;
    }
    ...
    fn work(runtime: *@import("queue_runtime.zig").Owner, raw: usize) void {
        ...
        for (&self.jobs) |*job| {
            ...
            if (job.ticket) |ticket| {
                _ = runtime.timeline.entry(ticket) catch { if (job.fence_retired) job.clear(); continue; };
        ...
        for (&self.jobs, 0..) |*job, index| if (job.owner == null) {
            ...
            job.owner = self; job.fence = input.fence;
            self.submit(job, index, input) catch {
                if (job.submitted) { runtime.timeline.fault(1); return; }
                if (job.ticket) |ticket| { runtime.timeline.cancelUnsubmitted(ticket) catch {}; }
                return;
            };
            return;
        };
    ...
    fn submit(self: *Owner, job: *Job, index: usize, input: a.GfxDriverJob) Error!void {
        const runtime = self.runtime.?; const memory = self.memory.?; const queue = runtime.queue.?;
        ...
        var count = try copy.encodeCopy(&self.commands, .{ .source = job.maps[0].address + input.source_offset,
            .target = job.maps[1].address + input.target_offset, .bytes = input.byte_length, .rows = rows,
            .source_pitch = input.source_pitch, .target_pitch = input.target_pitch });
        if (count >= self.commands.len) return error.Capacity;
        self.commands[count] = 0; count += 1;
        const ticket = try runtime.timeline.reserve(input.fence, .sdma, input.deadline_ns, .{ .context = @intFromPtr(job), .retire = Job.retire }); job.ticket = ticket;
        for (&job.maps, &job.retained) |*map, *held| { try map.retain(input.fence); held.* = true; }
        const ib = try runtime.arena.ib(ticket.slot); for (self.commands[0..count], 0..) |word, i| ib[i] = word;
        var commands: [32]u32 = undefined;
        _ = try @import("sdma_ring.zig").frame(&commands, self.engine.ring.write, arena_va + storage.ib_offset + @as(u64, ticket.slot) * storage.ib_bytes,
            count, try runtime.arena.address(storage.fence_offset + @as(usize, ticket.slot) * 8, 8), ticket.token, true);
...
const Test = struct {
    ...
    var pending = false; var active = false; var complete_fail = false; var release_fail = false;
    var unmapped: usize = 0; var unregistered: bool = false;
    var completed: u32 = 0; var last_result: u32 = 0; var time: u64 = 1000;
    var request: a.GfxDriverJob = .{};
    fn reset() !void {
        // These are static fixtures: no large state copy on a kernel-sized stack.
        owner = .{}; runtime = .{}; memory = .{}; regs = @splat(0); arena = @splat(0); bells = @splat(0);
        live = .{ false, false }; dma = .{ false, false }; gpu = .{ false, false };
        pending = false; active = false; unmapped = 0; unregistered = false; completed = 0; last_result = 0; complete_fail = false; release_fail = false; time = 1000;
        const context: r4os.driver_memory.Context = .{ .table = .{ .mmio_unmap = @intFromPtr(&unmap), .buffer_describe = @intFromPtr(&describe), .buffer_release = @intFromPtr(&release),
            .device_acquire = @intFromPtr(&acquire), .device_segment = @intFromPtr(&segment), .device_release = @intFromPtr(&releaseDevice), .collect = @intFromPtr(&collect) } };
        memory = .{ .self_address = @intFromPtr(&memory), .memory = context, .prepared = true, .adapter = 7, .epoch = 23,
        ...
        const mr = @import("memory_registers.zig"); regs[mr.gfx.VM_INVALIDATE_ENG17_ACK / 4] = 3; regs[mr.mm.VM_INVALIDATE_ENG17_ACK / 4] = 3;
        const binding: a.GfxBackendBinding = .{ .adapter_id = 7, .device_generation = 11, .reset_generation = 5 };
        runtime = .{ .self_address = @intFromPtr(&runtime), .prepared = true, .memory = &memory,
            .queue = .{ .table = .{ .unregister_backend = @intFromPtr(&unregister), .take = @intFromPtr(&take), .retain_resource = @intFromPtr(&retain), .complete = @intFromPtr(&complete) } } };
        runtime.arena = .{ .self_address = @intFromPtr(&runtime.arena), .memory = &memory, .epoch = 23, .gpu = 0x120000000, .ready = true,
            .arena = .{ .value = .{ .handle = .{ .id = 2, .generation = 1 }, .cpu_address = @intFromPtr(&arena), .byte_length = storage.bytes } },
        ...
        owner.self_address = @intFromPtr(&owner); owner.memory = &memory; owner.runtime = &runtime; owner.binding = binding; owner.registered = true; owner.verified = true; owner.queue = runtime.queue;
        owner.engine.ring = try @import("queue_ring.zig").Ring.init(try runtime.arena.words32(storage.ringOffset(.sdma), storage.ring_bytes), 8, 0); owner.engine.running = true;
        request = .{ .fence = .{ .adapter_id = 7, .device_generation = 11, .reset_generation = 5, .timeline = 3, .point = 1, .slot = 2 },
            .operation = a.gfx_queue_operation_copy_rows, .source_buffer = .{ .id = 1, .generation = 19 }, .target_buffer = .{ .id = 2, .generation = 19 },
            .byte_length = 64, .source_offset = 3, .target_offset = 11, .row_count = 5, .source_pitch = 80, .target_pitch = 100, .deadline_ns = 1_000_000 };
    ...
    fn unmap(_: *const a.GfxBufferHandle, quiet: u32) callconv(.c) i32 {
        ...
        if (release_fail) return -4; unmapped += 1; return 1;
    }
    fn unregister(_: *const a.GfxBackendBinding, quiet: u32) callconv(.c) i32 {
        ...
        unregistered = true; return 1;
    }
    fn now() callconv(.c) u64 { time += 100; return time; }
    fn collect() callconv(.c) i32 { return 1; }
    fn take(_: *const a.GfxBackendBinding, output: *a.GfxDriverJob) callconv(.c) i32 { if (!pending) return 0; pending = false; active = true; output.* = request; return 1; }
    fn retain(fence: *const a.GfxFence, side: u32, output: *a.GfxBufferReference) callconv(.c) i32 {
        std.debug.assert(active and side < 2 and !live[side] and std.meta.eql(fence.*, request.fence)); live[side] = true;
        output.* = .{ .reference = .{ .id = side + 10, .generation = 19 }, .buffer = if (side == 0) request.source_buffer else request.target_buffer }; return 1;
    }
    fn describe(reference: *const a.GfxBufferHandle, output: *a.GfxBufferDescriptor) callconv(.c) i32 {
        const side = reference.id - 10; std.debug.assert(side < 2 and live[side]); output.* = descriptors[side]; return 1;
    }
    fn release(reference: *const a.GfxBufferHandle) callconv(.c) i32 {
        const side = reference.id - 10; std.debug.assert(active and live[side] and !gpu[side] and !dma[side]);
        if (release_fail) return -4; live[side] = false; return 1;
    }
    fn acquire(reference: *const a.GfxBufferHandle, input: *const a.GfxDeviceRequest, output: *a.GfxDeviceLease) callconv(.c) i32 {
        ...
        if (input.access == 4) { std.debug.assert(!dma[side]); dma[side] = true; } else { std.debug.assert(input.access == 3 and !gpu[side]); gpu[side] = true; }
        output.* = .{ .lease = .{ .id = 100 + side * 10 + input.access, .generation = 19 }, .byte_length = input.byte_length, .gpu_virtual_address = input.gpu_virtual_address,
            .adapter_id = 7, .driver_owner = 9, .device_generation = 23, .access = input.access, .address_space = input.address_space, .dma_mask = input.dma_mask }; return 1;
    }
    fn segment(lease: *const a.GfxDeviceLease, offset: u64, output: *a.GfxDmaSegment) callconv(.c) i32 {
        const side = (lease.lease.id - 100) / 10; std.debug.assert(dma[side] and offset < descriptors[side].byte_length);
        const bytes = @min(@as(u64, 4096), descriptors[side].byte_length - offset);
        output.* = .{ .dma_address = 0x1000000 + @as(u64, side) * 0x100000 + offset * 3, .byte_length = bytes, .next_offset = offset + bytes }; return 1;
    }
    fn releaseDevice(lease: *const a.GfxDeviceLease, quiet: u32) callconv(.c) i32 {
        const side = (lease.lease.id - 100) / 10; std.debug.assert(active and quiet == 1);
        if (release_fail) return -4;
        if (lease.access == 3) { std.debug.assert(gpu[side]); gpu[side] = false; } else { std.debug.assert(dma[side] and !gpu[side]); dma[side] = false; } return 1;
    }
    fn complete(fence: *const a.GfxFence, result: u32, quiet: u32) callconv(.c) i32 {
        ...
        if (complete_fail) return -4; active = false; completed += 1; last_result = result; return 1;
    }
    fn submit() void { pending = true; Owner.work(&runtime, @intFromPtr(&owner)); }
    fn fenceWrite(token: u64) !void { const words = try runtime.arena.fences(); words[owner.jobs[0].ticket.?.slot] = token; }
};

test "SDMA common jobs retain exact BO tails and fences across stale completions abort and failed releases" {
    const t = std.testing;
    try Test.reset(); Test.submit();
    const job = &Test.owner.jobs[0]; const ticket = job.ticket.?;
    try t.expect(job.submitted and Test.live[0] and Test.live[1] and Test.memory.mapping_users == 2);
    try t.expectEqual(@as(u64, 5001), job.maps[0].bytes); try t.expectEqual(@as(usize, 2), job.maps[0].physical.len);
    ...
    try t.expectEqual(@as(u32, 3), ib[1]); try t.expectEqual(@as(u32, 0x0800000b), ib[6]);
    try t.expectEqual(@as(u32, 128), Test.bells[@import("queue_registers.zig").sdma_doorbell]);
    try Test.fenceWrite(ticket.token + 7); Test.runtime.timeline.poll(2000);
    try t.expect(Test.runtime.timeline.entries[ticket.slot].phase == .submitted and Test.active);
    var old = Test.request.fence; old.reset_generation += 1;
    try t.expect(!Job.retire(@intFromPtr(job), old)); try t.expect(Test.live[0] and Test.live[1]);
    try Test.fenceWrite(ticket.token); Test.runtime.timeline.poll(3000);
    Test.release_fail = true; try t.expect(!Test.runtime.timeline.publish(Test.runtime.queue.?));
    try t.expect(Test.active and Test.live[0] and Test.live[1] and Test.completed == 0);
    Test.release_fail = false; Test.complete_fail = true;
    ...
    Test.complete_fail = false; try t.expect(Test.runtime.timeline.publish(Test.runtime.queue.?));
    try t.expectEqual(@as(u32, 1), Test.completed); try t.expectEqual(a.gfx_queue_result_complete, Test.last_result);
    Owner.work(&Test.runtime, @intFromPtr(&Test.owner)); try t.expect(job.owner == null);
    // A missing TLB acknowledgement after PTE removal retains both leases.
    try Test.reset(); Test.submit(); const second = Test.owner.jobs[0].ticket.?;
    try Test.fenceWrite(second.token); Test.runtime.timeline.poll(4000);
    const mr = @import("memory_registers.zig"); Test.regs[mr.mm.VM_INVALIDATE_ENG17_ACK / 4] = 0;
    try t.expect(!Test.runtime.timeline.publish(Test.runtime.queue.?)); try t.expect(Test.gpu[0] and Test.dma[0] and Test.live[0]);
    Test.regs[mr.mm.VM_INVALIDATE_ENG17_ACK / 4] = 3; try t.expect(Test.runtime.timeline.publish(Test.runtime.queue.?));
    ...
    try Test.reset(); Test.submit(); Test.runtime.timeline.poll(Test.request.deadline_ns);
    ...
    try t.expect(Test.runtime.timeline.publish(Test.runtime.queue.?)); try t.expectEqual(a.gfx_queue_result_timeout, Test.last_result);
    ...
    for (0..4) |case| {
        ...
        Test.submit(); Owner.work(&Test.runtime, @intFromPtr(&Test.owner));
    ...
    Test.runtime.arena.arena.memory = Test.memory.memory; Test.runtime.arena.doorbell.memory = Test.memory.memory;
    ...
    try t.expect(!Test.owner.close()); Test.time += 100000;
    try t.expect(!Test.owner.close()); Test.time += 100000;
    ...
    Test.time += 1_000_000_000; Test.release_fail = false;
    try t.expect(Test.owner.close());
    ...
    try t.expect(Test.owner.close()); try t.expectEqual(@as(usize, 2), Test.unmapped);
}

test "SDMA startup requires real fill copy fence and consumed ring before provider activation" {
    const t = std.testing; const sr = @import("start_registers.zig").sdma;
    try Test.reset(); Test.owner.verified = false; Test.owner.registered = false;
    ...
    try t.expectEqual(@as(u32, 0x8000000b), ib[0]); try t.expectEqual(@as(u32, 1), ib[5]);
    data[0] = @truncate(Owner.test_token); data[1] = @truncate(Owner.test_token >> 32);
    ...
    try Test.reset(); Test.owner.verified = false; Test.owner.registered = false;
    try Test.owner.selftest(); Test.time += 2_000_000_000;
