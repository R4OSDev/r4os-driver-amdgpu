...
pub const Owner = struct {
    ...
    pub fn prepare(self: *Owner, owner: *jobs.Owner, native: *const @import("start_runtime.zig").Owner) c.Error!bool {
        ...
        if (!self.started) {
            ...
            self.started = true;
            self.restore_degraded = !native.flow.plan.restoreConfirmed();
            ...
            // Startup disabled GFXOFF and GC kept PG/SRM disabled. Without
            // authenticated restore images, preserve that awake state: no RLC
            // safe-mode, GC clock-gating or GFXOFF transaction is permitted.
            if (!self.restore_degraded) try self.gfx.begin(io, memory.layout.?.profile, !@import("power_gfx.zig").quirk(native.snapshot));
        }
        if (!self.restore_degraded) {
        ...
        const message = std.fmt.bufPrintZ(&text, "AMDGPU power: SMU=0x{x} factory-GFX={d}..{d}MHz limits={s} GFXOFF={s} APU-budget=firmware-owned", .{
            ...
            if (self.restore_degraded) "restore-unavailable-disabled" else if (self.gfx.gfxoff_supported) "idle-controlled" else "board-quirk-disabled" }) catch return error.Capacity;
    ...
    pub fn awake(self: *const Owner) bool {
        ...
        return !self.started or self.restored or (self.ready and (self.restore_degraded or self.gfx.awake()));
    }
    ...
    pub fn step(self: *Owner, owner: *jobs.Owner) c.Error!void {
        ...
        if (!self.restore_degraded) try self.gfx.step(io, busy, idle, false);
        ...
        const policy: u32 = if (self.restore_degraded) 8 else if (self.gfx.allowed) 0 else if (!self.gfx.awake()) 1 else if (retained(owner)) 4 else if (owner.client != null) 2 else 0;
