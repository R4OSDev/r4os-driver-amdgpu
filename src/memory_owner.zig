...
pub const Owner = struct {
    ...
    adapter: u32 = 0, epoch: u64 = 0, budget_live: bool = false, prepared: bool = false, mapping_users: u32 = 0,
    ...
    pub fn close(self: *Owner, gate: hubs.Gate) bool {
        if (self.self_address == 0) return true;
        if (self.self_address != @intFromPtr(self)) return false;
        if (self.mapping_users != 0 or self.orphan_release.cookie != 0 or self.virtual.mapped_pages != 0) return false;
