...
pub fn profile(native: *const @import("start_runtime.zig").Owner,
    ...
    return .{ .facts = copied,
        .timestamp_clock_khz = board.apuTimestampClock() catch 0,
...
pub fn capture(native: *const @import("start_runtime.zig").Owner,
    ...
    const asic = @import("asic_profile.zig").Profile.select(chip) catch return error.Unconfirmed;
    const firmware_profile = fw.select(&native.snapshot, chip) catch return error.Unconfirmed;
    if (!native.firmwareReady() or !store.valid or engine.phase != .ready or engine.faulted or engine.stop_started or
        layout.profile != asic or chip.external_revision != architecture.chip_revision or architecture.gc_version != chip.gc or architecture.sdma_version != chip.sdma or
        architecture.memory_generation != memory.epoch or architecture.gb_addr_config != engine.gb_addr_config or
        engine.cu_mask == 0 or engine.cu_mask & ~asic.cuMask() != 0 or engine.rb_mask == 0 or engine.rb_mask & ~asic.rbMask() != 0 or
    ...
    if (store.profile == null or !std.meta.eql(store.profile.?, firmware_profile)) return error.Unconfirmed;
    ...
    const parsed = fw.verify(topology, store.specification(.gpu_info) orelse return error.Unconfirmed) catch return error.Unconfirmed;
    ...
    if (limits[0] != 1 or limits[1] != @popCount(asic.cuMask()) or limits[2] != 1 or limits[3] != @popCount(asic.rbMask()) or limits[11] != 64 or
    ...
    return .{ .architecture = architecture, .version = 1, .size = @sizeOf(amd.R4AmdDeviceFacts),
        ...
        .pfp_fw = (store.specification(.pfp) orelse return error.Unconfirmed).ucode_version,
        .me_fw = (store.specification(.me) orelse return error.Unconfirmed).ucode_version,
        .mec_fw = (store.specification(.mec) orelse return error.Unconfirmed).ucode_version, .me_feature = me_feature, .mec_feature = mec_feature,
        .ce_fw = (store.specification(.ce) orelse return error.Unconfirmed).ucode_version, .smu_fw = native.flow.smu_version, .flags = 3,
