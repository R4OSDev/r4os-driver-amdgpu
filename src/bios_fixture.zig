...
pub fn set(comptime T: type, comptime name: []const u8, bytes: []u8, value: @FieldType(T, name)) void { put(@FieldType(T, name), bytes, bios.wireOffset(T, name), value); }
pub fn header(bytes: []u8, length: u16, major: u8, minor: u8) void { put(u16, bytes, 0, length); bytes[2] = major; bytes[3] = minor; }
pub fn entry(bytes: []u8, comptime name: []const u8, offset: u16) void { put(u16, bytes, 0x144 + bios.wireOffset(c.struct_atom_master_list_of_data_tables_v2_1, name), offset); }
...
pub fn rom(bytes: []u8) void {
    ...
    header(bytes[0x100..], bios.wireSize(R), 2, 2); @memcpy(bytes[0x104..0x108], "ATOM");
    ...
    header(bytes[0x140..], bios.wireSize(c.struct_atom_master_data_table_v2_1), 2, 1);
    ...
    header(bytes[0x340..], bios.wireSize(F), 3, 1); set(F, "bootup_sclk_in10khz", bytes[0x340..], 30000);
    entry(bytes, "integratedsysteminfo", 0x400); const I = c.struct_atom_integrated_system_info_v1_11;
    header(bytes[0x400..], 1024, 1, 11); set(I, "memorytype", bytes[0x400..], 0x1a); set(I, "umachannelnumber", bytes[0x400..], 2);
    ...
    header(bytes[0x900..], bios.wireSize(L), 2, 1); const timing = bytes[0x900 + bios.wireOffset(L, "lcd_timing")..];
    ...
    set(L, "panel_bpc", bytes[0x900..], 2); set(L, "max_allowed_bl_level", bytes[0x900..], 255);
    entry(bytes, "vram_usagebyfirmware", 0xa00); header(bytes[0xa00..], bios.wireSize(c.struct_vram_usagebyfirmware_v2_1), 2, 1);
